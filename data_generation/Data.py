...
RunnerInfo = namedtuple(
    'RunnerInfo', ['agent_type', 'error', 'total_act_time'])

GOAL_TEAM1 = [[-10.449999809265137, 0.07000000029802322, -64.5],
              [10.449999809265137, 0.07000000029802322, -64.5]]
GOAL_TEAM2 = [[10.460000038146973, 0.07000000029802322, 64.5],
...
class AIRunner:
    ...
    def new_match(self, team: int, num_players: int) -> list:
        return list(np.random.choice(KARTS, size=2))
    ...
    def new_match(self, team: int, num_players: int) -> list:
        ...
        try:
            ...
            if isinstance(r, str) or isinstance(r, list) or r is None:
                return r
            self._error = 'new_match needs to return kart names as a str, list, or None. Got {!r}!'.format(
                r)
        except Exception as e:
...
class Match:
    ...
    def __del__(self):
        # Don't ask why...
        if hasattr(self, '_pystk') and self._pystk is not None and self._pystk.clean is not None:
            self._pystk.clean()
    ...
    def _check(self, team1, team2, where, n_iter, timeout):
        ...
        if error:
            raise MatchException([0, 3], 'other team crashed',
                                 'crash during {}: {}'.format(where, error))

        _, error, t2 = self._g(self._r(team2.info)())
        if error:
            raise MatchException([3, 0], 'crash during {}: {}'.format(
                where, error), 'other team crashed')
    ...
    def run(self, team1, team2, num_player=1, max_frames=MAX_FRAMES, max_score=3, record_fn=None, timeout=1e10,
            initial_ball_location=[0, 0], initial_ball_velocity=[0, 0], verbose=False, data_callback=None, dump_every=10):
        ...
        # Deal with crashes
        t1_can_act, t2_can_act = self._check(
            team1, team2, 'new_match', 0, timeout)

        # Setup the race config
        logging.info('Setting up race')

        race_config = RaceConfig(
            track=TRACK_NAME, mode=RaceConfig.RaceMode.SOCCER, num_kart=2 * num_player)
        race_config.players.pop()
        for i in range(num_player):
            race_config.players.append(self._make_config(0, hasattr(
                team1, 'is_ai') and team1.is_ai, t1_cars[i % len(t1_cars)]))
            race_config.players.append(self._make_config(1, hasattr(
                team2, 'is_ai') and team2.is_ai, t2_cars[i % len(t2_cars)]))
        ...
        for it in range(max_frames):
            ...
            if it % dump_every == 0:
                for i in range(len(race.render_data)):
                    kart_mask = np.array(
                        (race.render_data[i].instance >> 24) == pystk.ObjectType.kart)
                    puck_mask = np.array(
                        (race.render_data[i].instance >> 24) == pystk.ObjectType.projectile)
                    b = np.zeros([300, 400])
                    # segmentation mask
                    mask = np.stack((255*kart_mask, 255*puck_mask, b), axis=-1)
                    ...
                    if np.dot(proj[2:3, 0:3], V[0:3].reshape([-1, 1])) > 0:
                        p = proj @ view @ np.array(
                            list(state.soccer.ball.location) + [1])
                        aim = np.array([p[0] / p[-1], -p[1] / p[-1]])
                        if np.abs(aim[0]) < 1 and np.abs(aim[1]) < 1:
                            x_puck = np.clip(
                                np.round(((aim[1] + 1) / 2) * 300), 0, 300)
                            y_puck = np.clip(
                                np.round(((aim[0] + 1) / 2) * 400), 0, 400)
                            puck.append([x_puck, y_puck])  # puck in view

                    k = []
                    for j in range(len(race.render_data)):
                        V = view @ np.array(
                            list(state.players[j].kart.location) + [1])
                        if np.dot(proj[2:3, 0:3], V[0:3].reshape([-1, 1])) > 0:
                            p = proj @ view @ np.array(
                                list(state.players[j].kart.location) + [1])
                            aim = np.array([p[0] / p[-1], -p[1] / p[-1]])
                    ...
                    data_callback(np.array(race.render_data[i].image), mask.astype(np.uint8), np.array(
                        puck, dtype=np.uint16), np.array(k, dtype=np.uint16), np.array(g, dtype=np.uint16), np.array(ic, dtype=np.uint16))

            if self._use_graphics:
                team1_images = [np.array(race.render_data[i].image)
                                for i in range(0, len(race.render_data), 2)]
                team2_images = [np.array(race.render_data[i].image)
                                for i in range(1, len(race.render_data), 2)]

            # Have each team produce actions (in parallel)
            if t1_can_act:
                if t1_type == 'image':
                    # team1_actions_delayed = self._r(team1.act)(team1_state, team1_images)
                    team1_actions_delayed = self._r(team1.act)(
                        team1_state, team1_images, state.soccer.ball.location)
                else:
                    team1_actions_delayed = self._r(team1.act)(
                        team1_state, team2_state, soccer_state)

            if t2_can_act:
                if t2_type == 'image':
                    team2_actions_delayed = self._r(team2.act)(
                        team2_state, team2_images, state.soccer.ball.location)
                else:
                    team2_actions_delayed = self._r(team2.act)(
                        team2_state, team1_state, soccer_state)

            # Wait for the actions to finish
            team1_actions = self._g(
                team1_actions_delayed) if t1_can_act else None
            team2_actions = self._g(
                team2_actions_delayed) if t2_can_act else None

            new_t1_can_act, new_t2_can_act = self._check(
                team1, team2, 'act', it, timeout)
            ...
            logging.debug(
                '  race.step  [score = {}]'.format(state.soccer.score))
...
if __name__ == '__main__':
    ...
    parser = ArgumentParser(
        description="Play some Ice Hockey. List any number of players, odd players are in team 1, even players team 2.")
    parser.add_argument('-r', '--record_video',
                        help="Do you want to record a video?")
    parser.add_argument('-s', '--record_state',
                        help="Do you want to pickle the state?")
    parser.add_argument('-o', '--output', default=DATASET_PATH)
    parser.add_argument('-f', '--num_frames', default=1200,
                        type=int, help="How many steps should we play for?")
    parser.add_argument('-p', '--num_players', default=2,
                        type=int, help="Number of players per team")
    parser.add_argument('-n', '--n_images', default=10000, type=int)
    parser.add_argument('-m', '--max_score', default=3,
                        type=int, help="How many goal should we play to?")
    parser.add_argument('-j', '--parallel', type=int,
                        help="How many parallel process to use?")
    parser.add_argument(
        '--ball_location', default=[0, 0], type=float, nargs=2, help="Initial xy location of ball")
    parser.add_argument(
        '--ball_velocity', default=[0, 0], type=float, nargs=2, help="Initial xy velocity of ball")
    parser.add_argument(
        'team1', help="Python module name or `AI` for AI players.")
    parser.add_argument(
        'team2', help="Python module name or `AI` for AI players.")
    ...
    if args.parallel is None or remote.ray is None:
        ...
        for i in range(args.parallel):
            ...
            if args.record_video:
                ...
                recorder = remote.RayVideoRecorder.remote(
                    args.record_video.replace(ext, f'.{i}{ext}'))
            elif args.record_state:
                ext = Path(args.record_state).suffix
                recorder = remote.RayStateRecorder.remote(
                    args.record_state.replace(ext, f'.{i}{ext}'))
