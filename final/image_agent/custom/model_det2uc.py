...
def Block(n_input_channels, n_output_channels):
    return torch.nn.Sequential(
        torch.nn.Conv2d(n_input_channels, n_output_channels,
                        kernel_size=(1, 3), padding=0),
        torch.nn.Conv2d(n_output_channels, n_output_channels,
...
class Detector(torch.nn.Module):
    # ,256]): 16, 32, 64, 128 n_class=3, kernel_size=3, use_skip=True)
    ...
    #     """
        self.conv1 = Block(
            3, layers[0])  # 1 to 32 we are getting heatmap per channel now
        ...
        # (H+2p-dil*(kern-1)-1)/str  +1  or dilation =3 kernel_size=2
        ...
        self.classifier = torch.nn.Conv2d(
            layers[0], 3, kernel_size=1)  # 2 diff classes
    ...
    def forward(self, x):
        ...
        b, c, h, w = x.shape
        x = (x - self.input_mean[None, :, None, None].to(x.device)
             ) / self.input_std[None, :, None, None].to(x.device)

        # (160,160)  # as an example images 128x96
        ...
        # 128x16x12             128x16x16           128x10x10                            128x40x40
    ...
    def detect(self, image, max_det=4):
        ...
        # .unsqueeze(0)) #image[None] shape is then (1 x 1 x h x w ) classes are puck and player
        hm = self.forward(image)
        all_lists = []
        # channel 0 : puck , channel 1: kart, channel 3, goal
        for channel in range(hm.size(dim=1)):
            peaks_per_object = []
            if channel == 0:
                current_list = extract_peak(
                    torch.sigmoid(hm[0][channel]), max_det=1)
            if channel == 1:
                current_list = extract_peak(
                    torch.sigmoid(hm[0][channel]), max_det=3)
            if channel == 2:
                current_list = extract_peak(
                    torch.sigmoid(hm[0][channel]), max_det=1)
            ...
            for each_object in current_list:
                ...
                peaks_per_object.append(
                    [int(each_object[1]), int(each_object[2])])
