                ...
                if np.abs(x-k[0])<=0.1 and np.abs(y-k[1])<0.25: #if you got the puck and close to the goals 
                  ...
                  if self.team == 0 and abs(kart_location[0]-10) <4 and kart_location[1]>62: #if close to opponent's goal and goal to the right side
                    ...
                    if np.dot(proj[2:3,0:3],v[0:3].reshape([-1,1])) > 0: 
                      ...
                      if np.abs(goal[0]) <= 1 and np.abs(goal[1]) <= 1:   #if you got the puck and the opponent's goal is in view 

                        if goal[0] <0: 

                          self.steer[i]=0.8 #turn the puck to the left 
                          self.brake[i]=True
                      ...
                      if np.dot(proj[2:3,0:3],v[0:3].reshape([-1,1])) > 0: 
                        ...
                        goal = np.array([p[0] / p[-1], -p[1] / p[-1]])
                        if np.abs(goal[0]) <= 1 and np.abs(goal[1]) <= 1: #if your own goal is in view 

                          if goal[0] <0: 

                            self.steer[i]=-0.8 #turn puck to left 
                            self.brake[i]=True
